...
class QuestionFlowService {
  ...
  QuestionSelectionResult selectQuestion({
    ...
    if (filteredQuestions.isEmpty) {
      ...
      if (allCategoryQuestions.isEmpty) {
        ...
        selectedQuestion =
            _pickQuestionPreferringShortLines(anyCategoryQuestions, random, useLineEstimation: useLineEstimation);
        logs.add(LogEntry(
        ...
        selectedQuestion =
            _pickQuestionPreferringShortLines(allCategoryQuestions, random, useLineEstimation: useLineEstimation);
        logs.add(LogEntry(
      ...
      selectedQuestion =
          _pickQuestionPreferringShortLines(filteredQuestions, random, useLineEstimation: useLineEstimation);
    }
  ...
  AnswerResult processAnswer({
    ...
    if (!isCorrect) {
      if (isBot) {
        logs.add(LogEntry(
          '🤖 Bot: Yanlış cevap. Yıldız kazanamadın.',
          type: 'error',
        ));
      } else {
        logs.add(LogEntry(
          'Yanlış cevap. Yıldız kazanamadın.',
          type: 'error',
        ));
      }
    ...
    final currentCount =
        player.getCorrectAnswerCount(categoryName, difficulty);
